...
class CustomHelp(commands.HelpCommand):
        ...
        for cog, command in mapping.items():
            ...
            if command_signatures:
                ...
                if cog_name != "No Category":
                    embed.add_field(
                        name=cog_name, value="".join(command_signatures))
    ...
    async def send_command_help(self, command):
        ...
        if filter:
            ...
            help.add_field(name=name, value="".join(
                command_embed), inline=True)
            help.add_field(name='Como usar: ',
                           value=get_content(content), inline=False)
            await ctx.send(embed=help)
        else:
            raise commands.BadArgument(
                "You probably don't have the Role to execute this command nor to get help of it")
